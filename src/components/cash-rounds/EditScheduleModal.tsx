...
interface EditScheduleModalProps {
  ...
  scheduleId: number;
  onSubmit: (scheduleId: number, rotationOrder: number[]) => void;
  isLoading?: boolean;
}
...
export default function EditScheduleModal({
  ...
  const [rotationOrder, setRotationOrder] = useState<number[]>(currentRotationOrder);

  // Update rotation order when modal opens with new data
  useEffect(() => {
    setRotationOrder(currentRotationOrder);
  }, [currentRotationOrder, isOpen]);
  ...
  const handleSubmit = () => {
    onSubmit(scheduleId, rotationOrder);
  };
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="Edit Rotation Schedule"
      size="md"
    >
      <div className="space-y-4">
        <p className="text-sm text-gray-600">
          Update the order in which members will receive payouts. Use the arrows to reorder members.
        </p>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
          <p className="text-sm text-amber-800">
            ⚠️ Changing the rotation order will affect future payouts. Current and completed payouts remain unchanged.
          </p>
        </div>
